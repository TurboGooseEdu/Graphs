...
def graph_deleted_largest_nodes(graph, number, nodes=None):
    if nodes is None:
...
def graph_deleted_largest_nodes_percentage(graph: UndirectedGraph, percentage, nodes=None):
    number = round(percentage * graph.v / 100)
    return graph_deleted_largest_nodes(graph, number, nodes[:number])
